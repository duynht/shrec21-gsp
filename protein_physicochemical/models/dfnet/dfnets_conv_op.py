...
def dfnets_graph_conv(x, num_filters, arma_conv_AR, arma_conv_MA, input_signal, ar_kernel, ma_kernel):
        ...
        conv_op_y1 = tf.keras.layers.concatenate(conv_op_y1, axis=1)
        conv_op_y1 = tf.keras.layers.dot(conv_op_y1, ar_kernel)
        
        conv_op_y2 = tf.split(y2, num_filters, axis=0)
        conv_op_y2 = tf.keras.layers.concatenate(conv_op_y2, axis=1)
        conv_op_y2 = tf.keras.layers.dot(conv_op_y2, ma_kernel)
